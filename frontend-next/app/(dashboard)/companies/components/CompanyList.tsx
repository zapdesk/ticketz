...
export default function CompanyList({ onEdit, refreshKey }: CompanyListProps) {
  ...
  const handleDelete = async (companyId: string) => {
    try {
      ...
      setCompanyList(prevList => prevList.filter(company => company.id !== companyId));
                ...
                <tr key={company.id}>
                  ...
                  <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                    ...
                    <button
                      ...
                      className={`text-red-600 hover:text-red-900 ${deletingId === company.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                      disabled={deletingId === company.id}
                    >
